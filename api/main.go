...
func main() {
	...
	sockRouter := mux.NewRouter()
	sockRouter.HandleFunc("/ws", wsPage)
	go http.ListenAndServe(":12345", sockRouter)

	router := mux.NewRouter()
	router.HandleFunc("/load", loadMessages).Methods("GET")
	router.HandleFunc("/send", persistMessage).Methods("POST")
